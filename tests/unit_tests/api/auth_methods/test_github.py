...
class TestGithub(TestCase):
    ...
    def test_map_team(self, test_label, mount_point, requests_mocker):
        expected_status_code = 204
        team_name = 'hvac'
        mock_url = 'https://127.0.0.1:8200/v1/auth/{mount_point}/map/teams/{team_name}'.format(
            mount_point=mount_point,
            team_name=team_name,
        )
        requests_mocker.register_uri(
    ...
    def test_map_user(self, test_label, mount_point, requests_mocker):
        expected_status_code = 204
        user_name = 'hvac'
        mock_url = 'https://127.0.0.1:8200/v1/auth/{mount_point}/map/users/{user_name}'.format(
            mount_point=mount_point,
            user_name=user_name,
        )
        requests_mocker.register_uri(
    ...
    def test_login(self, test_label, mount_point, requests_mocker):
        ...
        mock_url = 'https://127.0.0.1:8200/v1/auth/{mount_point}/login'.format(
            mount_point=mount_point,
        )
        requests_mocker.register_uri(
            method='POST',
