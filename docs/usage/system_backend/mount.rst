...
.. testcode:: sys_mount
    ...
    print('The following secrets engines are mounted: %s' % secrets_engines_list.keys())

Example output:

.. testoutput:: sys_mount

    The following secrets engines are mounted: dict_keys(['cubbyhole/', 'identity/', 'secret/', 'sys/'])
